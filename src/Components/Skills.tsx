...
function ListCardSkills(props: ListCardSkillsProps) {
  return (
    <div className={"m-4"}>
      {props.title ? <div className={"flex flex-row text-2xl mb-4 justify-center font-black"}>{props.title}</div> : null }
      <div
        className={"flex flex-col gap-y-4 gap-x-4 lg:gap-x-8 "}>
        {props.skills.map((skill, index) => (
          <div key={index} className={"flex flex-row gap-2 items-center"}>
            <img src={skill.img} alt={skill.name} className={"w-8 h-8 lg:w-8 lg:h-8"}/>
            <div className={"text-center font-bold"}>{skill.name}</div>
          </div>
        ))}
      </div>
    </div>
...
export default function Skills() {
  ...
  const tools : Skill[] = [
    ...
    {name: "UML", img: git},
    {name: "Markdown", img: git},
    {name: "Postman", img: git},
    {name: "Figma", img: git},
    {name: "Swagger", img: git},
    {name: "Cypress", img: git},
  ]
  ...
  return (
    <div id={"skills"} className={"frame flex-col"}>
      ...
      <div className={"flex flex-row mx-4 lg:mx-16"}>
        ...
        <ListCardSkills skills={environments} title={"Environnements"}/>
